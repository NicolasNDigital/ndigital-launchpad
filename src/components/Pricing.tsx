...
const Pricing = () => {
  ...
  const features = [{
    name: "Design moderne responsive",
    site: true,
    pack: true
  }, {
  ...
  return <section ref={ref} id="tarifs" className="py-20 md:py-32 bg-muted">
      <div className="container mx-auto px-4">
        ...
        delay: 0.2
          ...
          <div className="bg-card rounded-3xl shadow-xl overflow-hidden">
            ...
            <div className="divide-y divide-border">
              {features.map((feature, index) => <div key={index} className="grid grid-cols-3">
                  <div className="p-4 text-sm">{feature.name}</div>
                  <div className="p-4 flex justify-center items-center border-l border-border">
                    {feature.site ? <Check className="w-5 h-5 text-success" /> : <X className="w-5 h-5 text-muted-foreground" />}
                  </div>
        ...
        delay: 0.4
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {addons.map((addon, index) => <motion.div key={index} initial={{
            opacity: 0,
            scale: 0.9
          }} animate={isInView ? {
            opacity: 1,
